...
export interface ISchemaClient {
    schema_Index(records: Record[]): Observable<FileResponse>;
}

@Injectable({
    providedIn: 'root'
})
export class SchemaClient implements ISchemaClient {
    private http: HttpClient;
    private baseUrl: string;
    protected jsonParseReviver: ((key: string, value: any) => any) | undefined = undefined;
    ...
    schema_Index(records: Record[]): Observable<FileResponse> {
        ...
        return this.http.request("post", url_, options_).pipe(_observableMergeMap((response_ : any) => {
            return this.processSchema_Index(response_);
        })).pipe(_observableCatch((response_: any) => {
            if (response_ instanceof HttpResponseBase) {
                try {
                    return this.processSchema_Index(response_ as any);
                } catch (e) {
    ...
    protected processSchema_Index(response: HttpResponseBase): Observable<FileResponse> {
        const status = response.status;
        const responseBlob =
            response instanceof HttpResponse ? response.body :
            (response as any).error instanceof Blob ? (response as any).error : undefined;
...
export class Record implements IRecord {
    ...
    init(_data?: any) {
        if (_data) {
            ...
            this.consequence = _data["consequence"] ? Consequence.fromJS(_data["consequence"]) : <any>undefined;
        }
    }
    ...
    toJSON(data?: any) {
        ...
        data["consequence"] = this.consequence ? this.consequence.toJSON() : <any>undefined;
        return data;
    }
}
...
export class Consequence implements IConsequence {
    entity?: string | undefined;
    in?: string | undefined;
    plus?: Plus | undefined;

    constructor(data?: IConsequence) {
        if (data) {
            for (var property in data) {
                if (data.hasOwnProperty(property))
                    (<any>this)[property] = (<any>data)[property];
    ...
    init(_data?: any) {
        if (_data) {
            ...
            this.in = _data["in"];
            this.plus = _data["plus"] ? Plus.fromJS(_data["plus"]) : <any>undefined;
        }
    }

    static fromJS(data: any): Consequence {
        data = typeof data === 'object' ? data : {};
        let result = new Consequence();
        result.init(data);
        return result;
    }
